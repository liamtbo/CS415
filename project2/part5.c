...
int main(int argc, char *argv[]) {
    ...
    strcpy(print_table[0], "cpid\tstate\tsize\tuser\tkernel\tquantum\n");
    char initial_table[1024] = "0\t0\t0\t0\t0\t0\n";
    ...
    while (1) {
        ...
        strcat(process_stat_info, "\t");
        char quantum[10];
        sprintf(quantum, "%d", alarm_arr[process_i]);
        strcat(process_stat_info, quantum);
        strcat(process_stat_info, "\n");
        ...
        // if time spend in user mode (CPU) > time spent in kernal mode (IO)
        int user_time = atoi(cpid_token_buff.command_list[13]);
        int kern_time = atoi(cpid_token_buff.command_list[14]);
        if (user_time >= kern_time) {
            alarm_arr[process_i] = 2; // more relative time allocated
        } // if user mode < kernal mode 
        else {
            alarm_arr[process_i] = 1; // less relative time allocated
        }
...
/*
make an alarm array parallel to pid_arr
use proc/pid/stat to get process start
process time = clock() - start time
// update array for each
if kernal (io) time is more then 60% of total process time
    then give it less cpu time (1 second)
if user (cpu) time is mroe then 60% of total process tie
    then give it more cpu time (2 seconds)
else do nothing
// need to watch out for division by 0
