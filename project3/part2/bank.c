...
int main(int argc, char *argv[]) {
    ...
    // pthread_mutex_init(&transaction_count, NULL);
    ...
    // main thread waiting fro workert threads to be completed
    for(int i = 0; i < NUM_OF_WORKERS; i++){
...
void *update_balance() {
    // halt until lock if free
    // bank lock won't be free till all workers are done
