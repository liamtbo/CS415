...
int main(int argc, char *argv[]) {
    ...
    // change data format of ACC for memcpy
    account *format_acc = (account *)malloc(sizeof(account) * NUM_OF_ACC);
    for (int i = 0; i < NUM_OF_ACC; i++) {
        memcpy(&(format_acc[i]), ACC[i], sizeof(account));
    }
    // for (int i = 0; i < NUM_OF_ACC; i++) {
    //     printf("formated %d: %.2f\n", i, format_acc[i].balance);
    // }
    account *shared_memory = (account *)mmap(NULL, sizeof(account) * NUM_OF_ACC, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    memcpy(shared_memory, format_acc, sizeof(account)*NUM_OF_ACC);
    ...
    //     // array of account structs
        // account *puddles = (account *)malloc(sizeof(account) * NUM_OF_ACC);
        // for (int i = 0; i < NUM_OF_ACC; i++) {
            // strcpy(puddles[i].account_number, shared_memory[i].account_number);
            // puddles[i].balance = shared_memory[i].balance;
        // }
        // for (int i = 0; i < NUM_OF_ACC; i++) {
        //     printf("puddles %d: %.2f\n", i, puddles[i].balance);
        // }
